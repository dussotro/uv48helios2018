...
int main(int argc, char **argv){
    ...
    while(ros::ok()){
        ...
        for (i = 0; i<lenght_wpts-1; i++) {
            /*
            ab[0,0] = msg_waypoints->pose[i].x;
            ab[1,0] = msg_waypoints->pose[i].y;
            ab[0,1] = msg_waypoints->pose[i + 1].x;
            ab[1,1] = msg_waypoints->pose[i + 1].y;
            */
            ...
            msg_ab.pose[0].x = ab[0,0];
            msg_ab.pose[0].y = ab[1,0];
            msg_ab.pose[1].x = ab[0,1];
            msg_ab.pose[1].y = ab[1,1];

            d1 = ab[:, 1] - ab[:, 0];
            d2 = helios - ab[:, 0];
            e = d1[0, 0]*d2[1, 0] - d1[1, 0]*d2[0, 0]

            printf("e %f", e);


        std_msgs::Int64 msg_etat = "none";
        std_msgs::Int64 msg_num_wpts = 0;
